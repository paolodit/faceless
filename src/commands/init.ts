...
- story`);
  ...
  await Promise.all([
    ...
    writeTextFile(
      path.join(outputFolder, "SESSION_HANDOFF.md"),
      initialSessionHandoff(resolvedProjectName, projectArg, starter.pipeline)
    )
  ]);

  const type = getProductionPipeline(starter.pipeline);

  return `Created video-pack project: ${resolvedProjectName}

Creator type: ${type.title}

Fastest first run:
1. Replace ${projectArg}/input/script.txt with your spoken script.
...
Project guide:
${projectArg}/README_PROJECT.md

Session handoff:
${projectArg}/output/SESSION_HANDOFF.md`;
}

function initialSessionHandoff(
  projectName: string,
  projectArg: string,
  pipeline: ProjectStarter["pipeline"]
): string {
  const type = getProductionPipeline(pipeline);

  return `# Session Handoff

This file is generated by video-pack. Refresh it with \`video-pack board --project ${projectArg}\`.

## Project

- Name: ${projectName}
- Creator type: ${type.title}
- State: project created; spoken script and setup review are next
- Commands are relative to: ${process.cwd().replace(/\\/g, "/")}

## Resume

1. Replace \`${projectArg}/input/script.txt\` with the approved spoken script.
2. Run \`video-pack doctor --project ${projectArg}\`.
3. Run \`video-pack wizard --project ${projectArg}\`.
4. Continue with \`video-pack next --project ${projectArg}\`.

## Human Gates

- Confirm before any paid provider or external upload.
- Do not approve generated assets without human review.
- Do not replace creator-authored inputs without confirmation.
- Fact-check publishing copy and claims before release.
- This CLI does not publish automatically.
`;
}
...
If a coding agent is guiding this production, ask it to read output/SESSION_HANDOFF.md at the start of every returning session. The handoff is refreshed by next, proposal, package and board.
...
The concise return point is output/SESSION_HANDOFF.md. It records completed stages, current reviews, human gates and the exact next command.
