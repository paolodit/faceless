...
This file is generated by video-pack for a creator or coding agent returning to the production. Refresh it with \`video-pack board --project ${data.projectArg}\`.
...
2. Run \`video-pack doctor --project ${data.projectArg}\`.
3. Review the current files listed below.
4. Run the next command only after any required human decision.

\`\`\`bash
${data.nextCommand}
\`\`\`

${currentStage ? `Current stage: **${currentStage.label}** - ${currentStage.detail}.` : "All tracked stages are ready. Review the final package before publishing."}
