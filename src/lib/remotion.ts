...
async function copySceneMedia(
  ...
  for (const prompt of options.prompts) {
    const candidate = await findScenePrimaryAsset({
    ...
    const filename = safeAssetFilename(`${sceneFolderName(prompt.scene_number)}-${path.basename(candidate.filePath)}`);
    const assetFolder = candidate.kind === "video" ? "videos" : "images";
    const destination = path.join(destinationRoot, assetFolder, filename);
    writes.push(await copyOutputFile(candidate.filePath, destination, options.force));
    assets.set(prompt.image_filename, {
      src: `assets/${assetFolder}/${filename}`,
      filename,
      kind: candidate.kind
    });
  }
...
It turns scene media, optional stock assets, overlay events, captions and voiceover into a code-driven video draft.
...
- Missing scene media renders as readable placeholders.
- Scene media priority is: \`output/04_images/scenes/scene_XXX/video/clip.mp4\`, then \`upscaled/upscaled.png\`, then \`approved.png\`, then \`image.png\`, then the legacy \`output/04_images/full/\` file.
